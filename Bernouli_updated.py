...
train["PdDistrict"]= pd.get_dummies(train.PdDistrict)
train["DayOfWeek"] = pd.get_dummies(train.DayOfWeek)
train["Address"]= train["Address"].apply(lambda x: 1 if "/" in x else 0)
train["Month"] = pd.get_dummies(train.Month)
train["Day"] = pd.get_dummies(train.Day)
train["Hour"] = pd.get_dummies(train.Hour)
train["Minute"] = pd.get_dummies(train.Minute)
train["Year"] = pd.get_dummies(train.Year)
#add_encoder = LabelEncoder()
test["PdDistrict"] = pd.get_dummies(test.PdDistrict)
test["DayOfWeek"]= pd.get_dummies(test.DayOfWeek)
...
test["Month"] = pd.get_dummies(test.Month)
test["Day"] = pd.get_dummies(test.Day)
test["Hour"] = pd.get_dummies(test.Hour)
test["Minute"] = pd.get_dummies(test.Minute)
test["Year"] = pd.get_dummies(test.Year)
print(cat_encoder.classes_)
print(train.columns)
print(test.columns)
#Select only the required columns
train_columns = list(train.columns[2:12].values)
print(train_columns)
test_columns = list(test.columns[2:12].values)
print(test_columns)
#Split Coordinates:
